...
export default function Chat({ user, onLogout }) {
  ...
  async function loadUserSettings() {
    try {
      ...
      if (data) setUserSettings({ ...DEFAULT_SETTINGS, cooldownSeconds: data.cooldown_seconds || 3, dailyLimit: data.daily_limit || 5000 });
    } catch (err) {}
  }
  ...
  async function executeRequest(text, isRetry) {
    ...
    try {
      ...
      if (shouldSearch(text)) {
        ...
        if (searchResults) enhancedText = text + "\n\n[نتائج البحث من الإنترنت]:\n" + searchResults + "\n\nاستخدم النتائج دي في إجابتك.";
      }
      ...
      function type() {
        ...
        else { setStreamingText(""); setMessages(function(p) { return [...p, { role: "assistant", content: reply, id: Date.now() }]; }); setLoading(false); setTimeout(function() { inputRef.current?.focus(); }, 100); }
      }
      type();
  ...
  async function saveKeyUsage(keyId, newUsed) {
    try {
      ...
      if (existing && existing.length > 0) await supabase.from('user_usage').update({ tokens_used: newUsed }).eq('id', existing[0].id);
  ...
  if (isLoaded && keys.length === 0) {
    return (
      <div style={{ height: "100dvh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0f0f1a", color: "#e0e0e0", fontFamily: "system-ui, sans-serif", textAlign: "center", padding: "20px" }}>
        <div><div style={{ fontSize: "80px", marginBottom: "20px" }}>🔑</div><h2 style={{ fontSize: "24px", marginBottom: "15px" }}>مفيش مفاتيح متاحة</h2><p style={{ opacity: 0.7, fontSize: "16px", marginBottom: "10px" }}>تواصل مع المدير عشان يضيفلك مفتاح API</p><p style={{ opacity: 0.5, fontSize: "14px", marginBottom: "30px" }}>لما المفتاح يتضاف، دوس على الزر بالأسفل</p>
  ...
  return (
    <div className={`container ${isDark ? "dark" : "light"}`}>
      <div className="header"><div className="header-left"><div className="avatar">🖤</div><div><div className="header-name">بلاك</div><div className="header-status"><span className="status-dot" />{loading ? "بيكتب..." : "متصل"}</div></div></div><div className="header-right"><button onClick={newChat} className="header-btn" title="محادثة جديدة" style={{ fontSize: "20px" }}>➕</button><button onClick={function() { setShowMenu(!showMenu); }} className="header-btn" style={{ fontSize: "22px" }}>{showMenu ? "✕" : "☰"}</button></div>
        {showMenu && (<><div onClick={function() { setShowMenu(false); }} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, background: "rgba(0,0,0,0.5)" }} /><div style={{ position: "absolute", top: "60px", right: "10px", background: isDark ? "#1a1a2e" : "#fff", borderRadius: "16px", padding: "8px", zIndex: 201, display: "flex", flexDirection: "column", gap: "2px", minWidth: "220px", boxShadow: "0 10px 40px rgba(0,0,0,0.3)" }}><button onClick={function() { setShowHistory(!showHistory); setShowMenu(false); }} className="menu-item">💬 سجل المحادثات</button><button onClick={function() { setTheme(function(t) { return t === "dark" ? "light" : "dark"; }); }} className="menu-item">{isDark ? "☀️ الوضع النهاري" : "🌙 الوضع الليلي"}</button><button onClick={function() { onLogout(); }} className="menu-item" style={{ color: "#f87171" }}>🚪 تسجيل خروج</button></div></>)}
      </div>
      ...
      {showHistory && (<div className="search-bar" style={{ flexDirection: "column", alignItems: "stretch", gap: "8px", maxHeight: "250px", overflowY: "auto" }}><div style={{ display: "flex", justifyContent: "space-between" }}><strong>📝 سجل المحادثات</strong><button onClick={function() { setShowHistory(false); }} className="close-btn">✕</button></div>{allChats.length === 0 ? <div style={{ textAlign: "center", opacity: 0.6, padding: "10px" }}>مفيش محادثات</div> : allChats.map(function(c) { return (<div key={c.id} onClick={function() { openChat(c.id); }} style={{ display: "flex", justifyContent: "space-between", padding: "10px 14px", borderRadius: "12px", cursor: "pointer", background: c.id === currentChatId ? "rgba(108,92,231,0.2)" : "rgba(255,255,255,0.03)" }}><div style={{ flex: 1, overflow: "hidden" }}><div style={{ fontSize: "14px", fontWeight: 500 }}>{c.title}</div><div style={{ fontSize: "11px", opacity: 0.5 }}>{formatDate(c.date)} · {c.messageCount} رسالة</div></div><button onClick={function(e) { e.stopPropagation(); supabase.from('chats').delete().eq('id', c.id).then(loadChatsFromSupabase); }} style={{ background: "transparent", border: "none", color: "inherit", fontSize: "16px", cursor: "pointer", opacity: 0.5 }}>🗑️</button></div>); })}</div>)}
