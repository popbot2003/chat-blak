    ...
    <div style={{ minHeight: "100vh", background: "#0f0f1a", color: "#e0e0e0", fontFamily: "system-ui, sans-serif", padding: "20px" }}>
      ...
      {activeTab === "users" && (
        <div style={{ background: "#1a1a2e", borderRadius: "16px", padding: "20px", overflowX: "auto" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}><th style={{ padding: "12px", textAlign: "right" }}>المستخدم</th><th style={{ padding: "12px", textAlign: "right" }}>النموذج</th><th style={{ padding: "12px", textAlign: "right" }}>RPM</th><th style={{ padding: "12px", textAlign: "right" }}>يومي</th><th style={{ padding: "12px", textAlign: "right" }}>تبريد</th><th style={{ padding: "12px", textAlign: "right" }}>ذكي</th><th style={{ padding: "12px", textAlign: "right" }}>مفاتيح</th><th style={{ padding: "12px", textAlign: "right" }}>إجراءات</th></tr></thead>
            <tbody>
              {users.map(function(u) {
                return (
                  <tr key={u.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    <td style={{ padding: "12px" }}>{u.name}<br/><span style={{ fontSize: "11px", opacity: 0.5 }}>{u.email}</span></td>
                    <td style={{ padding: "12px", fontSize: "12px" }}>{u.selected_model || 'llama-3.1-8b-instant'}</td>
                    <td style={{ padding: "12px", fontSize: "13px" }}>{u.rate_limit_rpm || 5}</td>
                    <td style={{ padding: "12px", fontSize: "13px" }}>{(u.daily_limit || 5000).toLocaleString()}</td>
                    <td style={{ padding: "12px", fontSize: "13px" }}>{u.cooldown_seconds || 3}ث</td>
                    <td style={{ padding: "12px" }}>{u.smart_mode !== false ? '🧠' : '🎲'}</td>
                    <td style={{ padding: "12px" }}><span style={{ background: "rgba(108,92,231,0.2)", color: "#a29bfe", padding: "4px 10px", borderRadius: "20px", fontSize: "12px" }}>🔑 {getUserKeys(u.id).length}</span></td>
              ...
              {userKeys.map(function(key) {
                ...
                return (
                  <tr key={key.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    ...
                    <td style={{ padding: "12px", fontSize: "12px", fontFamily: "monospace" }}>{key.key_name}<br/>{key.key_value.slice(0, 20)}...</td>
      ...
      {activeTab === "chats" && (
        <div style={{ background: "#1a1a2e", borderRadius: "16px", padding: "20px", overflowX: "auto" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}><th style={{ padding: "12px", textAlign: "right" }}>المستخدم</th><th style={{ padding: "12px", textAlign: "right" }}>العنوان</th><th style={{ padding: "12px", textAlign: "right" }}>الرسائل</th><th style={{ padding: "12px", textAlign: "right" }}>آخر تحديث</th></tr></thead>
            <tbody>
      ...
      {showAddKeyModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
          <div style={{ background: "#1a1a2e", padding: "30px", borderRadius: "20px", width: "100%", maxWidth: "450px" }}>
            ...
            <label style={{ fontSize: "14px", opacity: 0.7, display: "block", marginBottom: "8px" }}>المستخدم</label>
            ...
            <label style={{ fontSize: "14px", opacity: 0.7, display: "block", marginBottom: "8px" }}>اسم المفتاح</label>
            ...
            <label style={{ fontSize: "14px", opacity: 0.7, display: "block", marginBottom: "8px" }}>قيمة المفتاح</label>
            ...
            <label style={{ fontSize: "14px", opacity: 0.7, display: "block", marginBottom: "8px" }}>الحد اليومي (token)</label>
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
          <div style={{ background: "#1a1a2e", padding: "30px", borderRadius: "20px", width: "100%", maxWidth: "500px", maxHeight: "80vh", overflowY: "auto" }}>
            ...
            <select value={editModel} onChange={function(e) { setEditModel(e.target.value); }} style={{ width: "100%", padding: "12px", marginBottom: "15px", borderRadius: "10px", background: "rgba(255,255,255,0.05)", color: "#e0e0e0", border: "1px solid rgba(255,255,255,0.1)", fontSize: "14px", outline: "none" }}>
              <option value="llama-3.1-8b-instant">🟢 llama-3.1-8b-instant (500K/يوم)</option>
              <option value="llama-3.3-70b-versatile">🟣 llama-3.3-70b-versatile (100K/يوم)</option>
            </select>

            <label style={{ fontSize: "14px", opacity: 0.7, display: "block", marginBottom: "8px" }}>📊 طلبات في الدقيقة (RPM)</label>
            ...
            <label style={{ fontSize: "14px", opacity: 0.7, display: "block", marginBottom: "8px" }}>💰 توكن في الدقيقة (TPM)</label>
            ...
            <label style={{ fontSize: "14px", opacity: 0.7, display: "block", marginBottom: "8px" }}>💵 الحد اليومي (توكن)</label>
            ...
            <label style={{ fontSize: "14px", opacity: 0.7, display: "block", marginBottom: "8px" }}>⏱️ فترة التبريد (ثواني)</label>
            ...
            <label style={{ fontSize: "14px", opacity: 0.7, display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px", cursor: "pointer" }}>
              ...
              🧠 التوزيع الذكي (يختار المفتاح الأقل استخداماً)
            </label>

            <div style={{ display: "flex", gap: "10px" }}>
              <button onClick={saveUserSettings} style={{ flex: 1, padding: "12px", background: "linear-gradient(135deg, #6c5ce7, #8b5cf6)", color: "#fff", border: "none", borderRadius: "10px", cursor: "pointer", fontSize: "14px", fontWeight: "bold" }}>💾 حفظ الإعدادات</button>
