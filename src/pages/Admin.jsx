                ...
                return (
                  <tr key={u.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    ...
                    <td style={{ padding: "12px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
                      ...
                      <button onClick={function() { supabase.from('profiles').update({ is_blocked: !u.is_blocked }).eq('id', u.id).then(loadUsers); }} style={{ padding: "6px 12px", background: u.is_blocked ? "rgba(74,222,128,0.2)" : "rgba(248,113,113,0.2)", color: u.is_blocked ? "#4ade80" : "#f87171", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "12px" }}>{u.is_blocked ? "فك" : "حظر"}</button>
                    </td>
                  </tr>
                );
              })}
                ...
                {allChats.map(function(chat) {
                  ...
                  return (
                    <tr key={chat.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                      <td style={{ padding: "12px" }}>{chatUser?.name || "غير معروف"}</td>
      ...
      {showChatViewer && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
          <div style={{ background: "#1a1a2e", padding: "30px", borderRadius: "20px", width: "100%", maxWidth: "600px", maxHeight: "80vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "20px" }}><h3>💬 محادثات {viewingUserName}</h3><button onClick={function() { setShowChatViewer(false); }} style={{ background: "transparent", border: "none", color: "#e0e0e0", fontSize: "20px", cursor: "pointer" }}>✕</button></div>
      ...
      {showAddKeyModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
          <div style={{ background: "#1a1a2e", padding: "30px", borderRadius: "20px", width: "100%", maxWidth: "450px" }}>
            ...
            <select value={selectedUser?.id || ""} onChange={function(e) { setSelectedUser(users.find(function(u) { return u.id === e.target.value; })); }} style={{ width: "100%", padding: "12px", marginBottom: "15px", borderRadius: "10px", background: "rgba(255,255,255,0.05)", color: "#e0e0e0", border: "1px solid rgba(255,255,255,0.1)", fontSize: "14px", outline: "none" }}><option value="">اختر مستخدم...</option>{users.map(function(u) { return <option key={u.id} value={u.id}>{u.name} ({u.email})</option>; })}</select>
      ...
      {showEditUserModal && selectedUser && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
          <div style={{ background: "#1a1a2e", padding: "30px", borderRadius: "20px", width: "100%", maxWidth: "500px", maxHeight: "80vh", overflowY: "auto" }}>
            ...
            <select value={editModel} onChange={function(e) { setEditModel(e.target.value); }} style={{ width: "100%", padding: "12px", marginBottom: "15px", borderRadius: "10px", background: "rgba(255,255,255,0.05)", color: "#e0e0e0", border: "1px solid rgba(255,255,255,0.1)", fontSize: "14px", outline: "none" }}><option value="llama-3.1-8b-instant">🟢 سريع</option><option value="llama-3.3-70b-versatile">🟣 ذكي</option></select>
            <input type="number" value={editRateLimitRPM} onChange={function(e) { setEditRateLimitRPM(parseInt(e.target.value) || 1); }} style={{ width: "100%", padding: "12px", marginBottom: "15px", borderRadius: "10px", background: "rgba(255,255,255,0.05)", color: "#e0e0e0", border: "1px solid rgba(255,255,255,0.1)", fontSize: "14px", outline: "none" }} placeholder="RPM" />
            <input type="number" value={editRateLimitTPM} onChange={function(e) { setEditRateLimitTPM(parseInt(e.target.value) || 100); }} style={{ width: "100%", padding: "12px", marginBottom: "15px", borderRadius: "10px", background: "rgba(255,255,255,0.05)", color: "#e0e0e0", border: "1px solid rgba(255,255,255,0.1)", fontSize: "14px", outline: "none" }} placeholder="TPM" />
            <input type="number" value={editDailyLimit} onChange={function(e) { setEditDailyLimit(parseInt(e.target.value) || 1000); }} style={{ width: "100%", padding: "12px", marginBottom: "15px", borderRadius: "10px", background: "rgba(255,255,255,0.05)", color: "#e0e0e0", border: "1px solid rgba(255,255,255,0.1)", fontSize: "14px", outline: "none" }} placeholder="حد يومي" />
            <input type="number" value={editCooldown} onChange={function(e) { setEditCooldown(parseInt(e.target.value) || 1); }} style={{ width: "100%", padding: "12px", marginBottom: "15px", borderRadius: "10px", background: "rgba(255,255,255,0.05)", color: "#e0e0e0", border: "1px solid rgba(255,255,255,0.1)", fontSize: "14px", outline: "none" }} placeholder="تبريد" />
            ...
            <div style={{ display: "flex", gap: "10px" }}><button onClick={saveUserSettings} style={{ flex: 1, padding: "12px", background: "linear-gradient(135deg, #6c5ce7, #8b5cf6)", color: "#fff", border: "none", borderRadius: "10px", cursor: "pointer", fontSize: "14px", fontWeight: "bold" }}>💾 حفظ</button><button onClick={function() { setShowEditUserModal(false); }} style={{ flex: 1, padding: "12px", background: "rgba(255,255,255,0.05)", color: "#e0e0e0", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", cursor: "pointer", fontSize: "14px" }}>إلغاء</button></div>
          </div>
        </div>
      )}
    </div>
