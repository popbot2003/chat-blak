...
export default function App() {
    ...
    try {
      ...
      if (!error && data) {
        ...
        if (data.date === today) {
          ...
          setKeys(freshKeys);
          await supabase.from('token_usage').upsert({ id: 1, date: today, total_used: 0, keys_data: [], updated_at: new Date().toISOString() });
        }
      }
    } catch {}
  };
  ...
  const saveTokenToCloud = async () => {
    try {
      await supabase.from('token_usage').upsert({
        ...
        keys_data: keysRef.current.map(k => ({ id: k.id, used: k.used, last: k.last })),
  ...
  const saveChatToCloud = async () => {
    try {
      ...
      if (data) setAllChats(data.map(c => ({ id: c.id, title: c.title || 'محادثة', date: c.updated_at, messageCount: c.messages?.length || 0 })));
    } catch {}
  };
  ...
  useEffect(() => { const h = () => { if (isLoaded) { saveChatToCloud(); saveTokenToCloud(); } }; window.addEventListener("beforeunload", h); return () => window.removeEventListener("beforeunload", h); }, [isLoaded]);
  ...
  const resetTokenCounter = () => { if (window.confirm("تصفر؟")) { setTokenData({ used: 0, date: new Date().toDateString() }); const fk = keysRef.current.map(k => ({ ...k, used: 0 })); setKeys(fk); saveTokenToCloud(); setShowMenu(false); } };
  ...
  const sendMessage = async (text, retry = false) => {
    ...
    if (!picked) { setMessages(p => [...p, { role: "assistant", content: "خلصت كل المفاتيح 😅", id: Date.now() }]); setLoading(false); return; }
    ...
    try {
      ...
      addTokens({ prompt_tokens: pt, completion_tokens: ct });
